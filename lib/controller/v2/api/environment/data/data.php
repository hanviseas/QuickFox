 ...
 * Controller 获取环境数据源
 */
class Controller_V2_Api_Environment_Data_Data extends Controller_V2_Api_Environment_Base
{
	...
	public function act()
	{
		...
		if ($this->params['order_field'] !== '') {
			$order_field = in_array($this->params['order_field'], array('id', 'source', 'keyword')) ? $this->params['order_field'] : 'id';
			$order_type = in_array($this->params['order_type'], array('asc', 'desc')) ? $this->params['order_type'] : 'asc';
			$order = array($order_field => $order_type);
		}
		$data = LF\M('V2.Environment.Data')->get_by_environment($this->params['environment_id'], $this->params['page'], $this->params['size'], $where, $order);
